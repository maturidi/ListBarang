...
<body>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			...
			<li class="active"><a href="<?php echo base_url(); ?>cuser"><em class="fa fa-clone">&nbsp;</em> User</a></li>
                    ...
                    <div class="panel panel-info">
                        ...
                        <!-- /.panel-heading -->
                         ...
                         <div class="panel-body">
                            <form action="<?php echo base_url(); ?>Cuser/edit/<?php if($row){ echo $row->IDUser; }else{ echo $id; } ?>" method="POST">
								<div class="form-group">
									<label>ID User</label>
									<input type="number" readonly value="<?php if($row){ echo $row->IDUser; }else{ echo set_value('id'); }?>" class="form-control" name="id" placeholder="ID User"></input>
									<?php echo form_error('id', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Nama User</label>
									<input type="text" value="<?php if($row){ echo $row->NamaUser; }else{ echo set_value('nama'); }?>" class="form-control" name="nama" placeholder="Nama User"></input>
								...
								<div class="form-group">
									...
									<input type="password" value="<?php if($row){ echo $row->Password; }else{ echo set_value('password'); }?>" class="form-control" name="password" placeholder="Password"></input>
								...
								<div class="form-group">
									...
									<select name="role" class="form-control">
										...
										foreach($role->result() as $row) {
											if ($row->IDRole == set_value('role')) {
												echo '<option value="'. $row->IDRole .'" selected>'. ucwords($row->NamaRole) .'</option>';
											} else {
												echo '<option value="'. $row->IDRole .'">'. ucwords($row->NamaRole) .'</option>';
											}
										}
										?>
									</select>
									<?php echo form_error('role', '<p style="color: #a94442">', '</p>'); ?>
								</div>
